...
def sync_data(
    ...
    for _env_enum, env_class in environments_to_sync:
        ...
        # Pre-filter tables to sync for this environment to avoid confusing logs
        tables_for_this_env = []
        for table_name, (model_class, emplacement_class) in (
            env_instance.table_situation.items()
        ):
            if not model_class:
                continue

            if tables_to_sync_map is not None:
                if (str(_env_enum), str(table_name)) in tables_to_sync_map:
