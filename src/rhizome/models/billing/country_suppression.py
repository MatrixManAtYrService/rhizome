"""
SQLModel definition for the country_suppression table.

This module provides the SQLModel class for the country_suppression table from the
...
class CountrySuppression(RhizomeModel, table=False):
    """
    SQLModel for the `country_suppression` table.

    This model represents country_suppression records in the billing system.
    """
    ...
    def sanitize(self) -> CountrySuppression:
        """Return a sanitized copy of this CountrySuppression instance."""
        return CountrySuppression(
            id=self.id,
