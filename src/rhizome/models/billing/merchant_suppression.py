"""
SQLModel definition for the merchant_suppression table.

This module provides the SQLModel class for the merchant_suppression table from the
...
class MerchantSuppression(RhizomeModel, table=False):
    """
    SQLModel for the `merchant_suppression` table.

    This model represents merchant_suppression records in the billing system.
    """
    ...
    finalize: bool | None = Field(default=None, description="finalize")

    def sanitize(self) -> MerchantSuppression:
        """Return a sanitized copy of this MerchantSuppression instance."""
        return MerchantSuppression(
            id=self.id,
